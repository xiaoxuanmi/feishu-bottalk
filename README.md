...
│   │   ├── src/
│   │   └── skills/
│   └── feishu-bottalk/                    # feishu-bottalk 插件（自定义，需手动部署）
│       ├── openclaw.plugin.json
│       ├── index.ts
│       ├── package.json
│       └── src/
...
| 配置事件订阅age | 选择使用 websocket 连接，订阅 `im.message.receive_v1`                        |
| 发布应用      | 提交审核并发布                                                             |
...
### 3.2 feishu-bottalk 插件

feishu-bottalk 插件需要手动部署到 `~/.openclaw/extensions/feishu-bottalk/`。将插件代码放入该目录，确保包含以下文件：

```
~/.openclaw/extensions/feishu-bottalk/
├── openclaw.plugin.json   # 插件 manifest，声明插件 ID、描述、工具
├── index.ts               # 主入口，注册 feishu-bottalk 工具
├── package.json           # npm 依赖
└── src/
    ├── types.ts           # 类型定义
...
**注意**：飞书 API 参数名是 **snake\_case**（`app_id`、`app_secret`），但配置文件中用 camelCase（`appId`、`appSecret`），由 OpenClaw 内部转换。
...
`tools.sessions.visibility = "all"` 和 `tools.agentToAgent.enabled = true` 是 agent\_say 插件 `session-inject.ts` 能注入消息的前提。
...
{
  "plugins": {
    "allow": ["feishu", "feishu-bottalk"],      // 允许加载的插件列表
    "entries": {
      "feishu": {
        "enabled": true
      },
...
- `allow` — 声明允许加载哪些插件。必须包含 `feishu` 和 `feishu-bottalk`。
- `entries` — 每个插件的启用状态和配置。feishu-bottalk 的 `timeoutSeconds: 0` 是 fire-and-forget 模式，使插件不阻塞等待结果。
...
**你只能通过 feishu-bottalk 工具调用来回复消息，绝对不允许直接输出文本回复。**

收到消息后，你的行为只有两个选择：
1. **调用 feishu-bottalk 工具** — 这是你回复的唯一方式
...
- `feishu-bottalk: loaded` — agent\_say 插件加载成功
- 3 个 agent（main / rd / pm）注册成功

### 6.3 用 HTTP API 验证 agent\_say
...
curl -s -X POST http://127.0.0.1:18789/tools/invoke \
  ...
  -d '{
    "tool": "feishu-bottalk",
    "args": {
      "message": "🧪 测试消息：feishu-bottalk 链路验证",
      "chat_id": "你的群聊chat_id",
      "agent_id": "rd"
    }
  }' | jq .
...
1. 在飞书群里 @研发小工 发消息
2. 确认 rd 通过 agent\_say 回复，群聊可见
3. 确认 pm 收到注入消息（日志中可见）

### 6.5 测试 Agent 间通信

1. 在群里说"帮我 @产品小理 问一下下周有没有需求评审"
2. rd 用 agent\_say 发消息，pm 收到并回复
...
| 症状                        | 原因                                            | 修复                                                      |
| ------------------------- | --------------------------------------------- | ------------------------------------------------------- |
| agent\_say 工具不存在          | `tools.allow` 被 `stripPluginOnlyAllowlist` 删掉 | 确保 `tools.allow = ["*", "group:plugins"]`，必须有 `"*"`     |
| agent 直接写文本回复             | AGENTS.md 的回复协议不够明确                           | 明确写"绝对不允许直接输出文本回复"，列出禁止事项                               |
| 消息循环（无限重复）                | agent 用 sessions\_send 回复注入消息                 | AGENTS.md 加防循环规则                                        |
...
- [ ] 3 个机器人已拉入群聊
- [ ] `~/.openclaw/extensions/feishu-bottalk/` 目录已部署
- [ ] 通过 `openclaw agents add` 添加 rd 和 pm agent
- [ ] `openclaw.json` 中每个 agent 的 `tools.allow` 含 `"*"`
- [ ] `openclaw.json` 中 `plugins.allow` 包含 `feishu` 和 `feishu-bottalk`，`entries` 中均已启用
