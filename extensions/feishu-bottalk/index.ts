...
import type { FeishuBottalkConfig } from "./src/types.js";
import { sendFeishuMessage } from "./src/feishu-client.js";
import { injectSessionMessage } from "./src/session-inject.js";

const TOOL_NAME = "feishu-bottalk";

function resolvePluginConfig(api: OpenClawPluginApi): FeishuBottalkConfig {
  const raw = (api.config?.plugins?.entries as Record<string, unknown>)?.["feishu-bottalk"];
  if (raw && typeof raw === "object" && "config" in raw) {
    return ((raw as Record<string, unknown>).config ?? {}) as FeishuBottalkConfig;
  }
  return {};
}
...
export default function register(api: OpenClawPluginApi) {
  ...
  api.logger.info?.("feishu-bottalk: Registered feishu-bottalk tool");
}
